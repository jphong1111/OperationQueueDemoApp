...
extension UIView {
    ...
    func addConstrainedSubView(_ view: UIView,
        ...
        let constraints = [
            constraint((fromView: view.leadingAnchor, toView: self.leadingAnchor),
                       constant: insets.left,
                       isLoose: ends.contains(.leading)),
            constraint((fromView: view.topAnchor, toView: self.topAnchor),
                       constant: insets.top,
                       isLoose: ends.contains(.top)),
            constraint((fromView: self.trailingAnchor, toView: view.trailingAnchor),
                       constant: insets.right,
                       isLoose: ends.contains(.trailing)),
            constraint((fromView: self.bottomAnchor, toView: view.bottomAnchor),
    ...
    private func constraint<AnchorType: AnyObject>(_ anchors:(fromView: NSLayoutAnchor<AnchorType>,
        ...
        isLoose ? anchors.fromView.constraint(greaterThanOrEqualTo: anchors.toView,
                                              constant: constant) : anchors.fromView.constraint(equalTo: anchors.toView,
                                                                                                constant: constant)
    }
}
